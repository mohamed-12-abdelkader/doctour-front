...
type StaffRoleForm = 'secretary' | 'doctor'

function normalizeStaffList(data: unknown): Staff[] {
    if (Array.isArray(data)) return data as Staff[]
    if (data && typeof data === 'object' && 'staff' in data && Array.isArray((data as { staff: unknown }).staff)) {
        return (data as { staff: Staff[] }).staff
    }
    if (data && typeof data === 'object' && 'users' in data && Array.isArray((data as { users: unknown }).users)) {
        return (data as { users: Staff[] }).users
    }
    return []
}
...
export default function AccountsPage() {
    ...
    const [formRole, setFormRole] = useState<StaffRoleForm>('secretary')
    ...
    const fetchStaff = async () => {
        ...
        try {
            ...
            setStaffList(normalizeStaffList(res.data))
        } catch (error) {
    ...
    const handleOpenEdit = (staff: Staff) => {
        ...
        setFormRole(isDoctor ? 'doctor' : 'secretary')
        setSpecialty(staff.doctorProfile?.specialty?.trim() ?? '')
        setPhone(staff.doctorProfile?.phone?.trim() ?? '')
        setImageUrl(staff.doctorProfile?.imageUrl?.trim() ?? '')
    ...
    const handleSubmit = async () => {
        ...
        if (!nameTrim) {
            showNotification('warning', 'أدخل الاسم')
            return
        }
        if (!emailTrim) {
            showNotification('warning', 'أدخل البريد الإلكتروني')
            return
        }
        if (!editingStaff) {
            if (!password) {
                showNotification('warning', 'أدخل كلمة المرور')
                return
            }
            if (formRole === 'doctor') {
                if (!specialty.trim()) {
                    showNotification('warning', 'أدخل التخصص للطبيب')
                    return
                }
                if (!phone.trim()) {
                    showNotification('warning', 'أدخل رقم الهاتف للطبيب')
                    return
                }
            }
        }
        if (editingStaff?.role === 'doctor') {
            if (!specialty.trim()) {
                showNotification('warning', 'أدخل التخصص')
                return
            }
            if (!phone.trim()) {
                showNotification('warning', 'أدخل رقم الهاتف')
                return
            }
        }
        ...
        try {
            if (editingStaff) {
                ...
                showNotification('success', 'تم تحديث الحساب بنجاح')
            } else {
                const payload: Record<string, unknown> = {
                    name: nameTrim,
                    email: emailTrim,
            ...
            showNotification('error', typeof msg === 'string' ? msg : 'فشلت العملية')
        } finally {
                        ...
                        <Flex align="center" gap={4}>
                            ...
                            <Box>
                                ...
                                <Text color="whiteAlpha.900" fontSize="sm" mt={1}>
                                    إضافة وتعديل حسابات الأطباء والسكرتارية والصلاحيات
                                </Text>
                            </Box>
                        </Flex>
                        <Button
                                                    ...
                                                    <Box>
                                                        ...
                                                        {staff.role === 'doctor' && staff.doctorProfile && (
                                                            <Text fontSize="xs" color="gray.500" mt={0.5}>
                                                                {staff.doctorProfile.specialty || '—'}
                                                                {staff.doctorProfile.phone ? ` · ${staff.doctorProfile.phone}` : ''}
                                                            </Text>
                                                        )}
                                                    </Box>
                                                </Flex>
                    ...
                    <Dialog.Content dir="rtl" borderRadius="2xl" overflow="hidden">
                        <Dialog.Header bg="#fdfbf7" borderBottom="1px solid" borderColor="gray.100" py={4} px={6}>
                            <Dialog.Title fontSize="lg" fontWeight="bold" color="#615b36">
                                {editingStaff ? 'تعديل حساب' : 'إضافة طبيب أو سكرتير'}
                            </Dialog.Title>
                            <Dialog.CloseTrigger />
                        </Dialog.Header>
                        <Dialog.Body p={6}>
                            <Stack gap={4}>
                                {!editingStaff ? (
                                    <Field.Root required>
                                        <Field.Label>نوع الحساب</Field.Label>
                                        <Box
                                            bg="gray.50"
                                            border="1px solid"
                                        ...
                                        >
                                            <select
                                                value={formRole}
                                                onChange={(e) => {
                                                    const r = e.target.value as StaffRoleForm
                                                    setFormRole(r)
                                                    if (r === 'secretary') {
                                                        setSpecialty('')
                                                        setPhone('')
                                ...
                                <Field.Root required>
                                    ...
                                    <Input
                                        placeholder="مثال: د. أحمد أو اسم السكرتير"
                                        value={name}
                                ...
                                {((!editingStaff && formRole === 'doctor') ||
                                    (!!editingStaff && editingStaff.role === 'doctor')) && (
                                    <>
                                        <Field.Root required>
                                            <Field.Label>التخصص</Field.Label>
                                            <Input
                                        ...
                                        <Field.Root required>
                                            <Field.Label>هاتف الطبيب</Field.Label>
                                            <Input
                                        ...
                                        <Field.Root>
                                            <Field.Label>رابط الصورة (اختياري)</Field.Label>
                                            <Input
                                                placeholder="https://..."
                                                value={imageUrl}
